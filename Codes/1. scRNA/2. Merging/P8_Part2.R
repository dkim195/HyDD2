...
#Lhx2
load(file = "Robj/P8_Lhx2_Neuron.Robj")
P8_Lhx2$SCT_snn_res.0.8 <- NULL
P8_Lhx2$old.ident <- NULL
P8_Lhx2$seurat_clusters <- NULL
Idents(P8_Lhx2) <- "Age"
P8_Lhx2 <- RenameIdents(P8_Lhx2, "TKD5" = "Mutant", "TKD6" = "Mutant")
P8_Lhx2 <- AddMetaData(P8_Lhx2, P8_Lhx2@active.ident, "Reference")
P8_Lhx2 <- AddMetaData(P8_Lhx2, P8_Lhx2@active.ident, "Cluster_Pass3")
Idents(P8_Lhx2) <- "Cluster_Pass3"

####transfer method####
#error in 3.1.5 but should be fixed in v3.2.3
#Error in `.rowNamesDF<-`(x, value = value) : invalid 'row.names' length
options(future.globals.maxSize = 8000 * 1024^2)

Mutant.anchors <- FindTransferAnchors(reference = P8, query = P8_Lhx2,
                                      normalization.method = "SCT",
                                      reduction = "pcaproject")
predictions <- TransferData(anchorset = Mutant.anchors,
                            refdata = P8$Cluster_Pass3, dims = 1:30)
P8_Lhx2 <- AddMetaData(P8_Lhx2, metadata = predictions)

P8_Lhx2$prediction.match <- P8_Lhx2$predicted.id == P8_Lhx2$orig.ident
table(P8_Lhx2$prediction.match) ##0 as PC0 is similar as PR0
DimPlot(P8_Lhx2, reduction = "umap", group.by = "predicted.id", label=T)
P8_Lhx2@meta.data$Cluster_Pass3 <- P8_Lhx2@meta.data$predicted.id
...
#For Lhx2
P8 <- subset(P8, subset = nCount_RNA < 6000)
P8 <- subset(P8, subset = nFeature_RNA < 4000)
#For Lhx1


#For Dlx1
#P8 <- P8[, sample(colnames(P8), size =8000, replace=F)]
#P8 <- P8[, sample(colnames(P8), size = 20000, replace=F)]

Idents(P8) <- "Cluster_Pass3"
Mutant <- merge(x = P8, y = list(P8_Lhx2))
...
save(Mutant, file = "Robj/P8_Lhx2_Final.Robj")
...
library(harmony)
setwd("E:/")
load(file = "Robj/P8_Lhx2_Final.Robj")
...
FeaturePlot(Mutant, "Lhx2", split.by = "Genotype", order = T )
...
#Lhx1
save(Mutant, file = "Robj/P8_Lhx1_Final.Robj")
...
VlnPlot(Mutant, c("Avp","Gal","Calb1","Oxt","Otp","Meis2","Gad1","Zic1","Zic4",
                  "Zic2","Rax","Foxg1"), group.by = "Genotype") + NoLegend()
