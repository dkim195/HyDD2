...
# Set up logging
logging.basicConfig(filename="pycistopic_OBJECT_Ctrl.log", level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

logging.info("Script started.")

# Change to the appropriate directory
os.chdir("/faststorage/project/Hypothalamus/SCENIC/ATAC/OUTPUT")

# Verify that the OBJECT directory exists
if not os.path.exists("OBJECT"):
    logging.error("The directory 'OBJECT' does not exist.")
    raise FileNotFoundError("The directory 'OBJECT' does not exist.")

# Path to the fragment matrix
matrix_path = "OBJECT/OBJECT_count_matrix.feather"
...
cell_annotation_path = "OBJECT/OBJECT_cell_annotation.tsv"
...
out_dir = "OBJECT/OBJECT_outs"
os.makedirs(out_dir, exist_ok=True)
pickle.dump(cisTopic_obj, open(os.path.join(out_dir, "OBJECT_obj.pkl"), "wb"))

# Configure memory for MALLET
os.environ['MALLET_MEMORY'] = '400G'  # Increase MALLET memory allocation
...
pickle.dump(models, open(os.path.join(out_dir, "OBJECT_models.pkl"), "wb"))
...
X_umap = pd.read_csv("OBJECT/OBJECT_cell_embeddings.csv", index_col=0)
X_umap.columns = ["UMAP_1", "UMAP_2"]
X_umap.index = [idx + '___OBJECTic' for idx in X_umap.index]
