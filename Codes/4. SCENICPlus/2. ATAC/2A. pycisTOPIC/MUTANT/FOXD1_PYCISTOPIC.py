...
# Set up logging
logging.basicConfig(filename="pycistopic_Foxd1.log", level=logging.INFO,
                    format='%(asctime)s - %(levelname)s - %(message)s')

logging.info("Script started.")

# Change to the appropriate directory
os.chdir("/faststorage/project/Hypothalamus/SCENIC/ATAC/OUTPUT/FOXD1")

logging.info(f"Changed to directory: {os.getcwd()}")

# Path to the fragment matrix
matrix_path = "Mutant_Foxd1_count_matrix.feather"
...
cell_annotation_path = "Mutant_Foxd1_cell_annotation.tsv"
...
pickle.dump(cisTopic_obj, open(os.path.join(out_dir, "Foxd1_obj.pkl"), "wb"))
...
pickle.dump(models, open(os.path.join(out_dir, "Foxd1_models.pkl"), "wb"))
...
X_umap = pd.read_csv("Mutant_Foxd1_cell_embeddings.csv", index_col=0)
