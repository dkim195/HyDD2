...
selected_eRegulons <- c("Dlx1_extended_+/+", "Dlx1_extended_-/+",
                        "Dlx2_extended_+/+", "Dlx2_extended_-/+")

filtered_data <- filtered_data %>%
  filter(eRegulon_name %in% selected_eRegulons)

# Combine 'Dlx_extended_+/+' and 'Dlx_extended_-/+' into 'Dlx'
filtered_data <- filtered_data %>%
...
# Print the plot
print(p)
postscript(file = "Figures/Dlx/SCENIC_Dlx_eRegulon_Pathway_Regulon_TF.eps", width = 30, height = 15,
...
####Dot plot####
write.csv(filtered_data, "Figures/Dlx/Dlx_filtered_data.csv", row.names = FALSE)
...
dat <- melt(graph)
write.csv(dat, file = "Figures/Dlx/dat.csv", row.names = F)
...
ggsave("Figures/Dlx/SCENIC_Dlx_dotplot.eps", plot = dot_plot, width = 20, height = 8, units = "in")
print(dot_plot)
dev.off()
...
library(grDevices)
eps_file <- "Figures/Dlx/Dlx_heatmap.eps"
setEPS()
...
clusters <-c("15","2","1","17","3","4","12","11","10","13")
for (cluster in clusters) {
  ...
  file_name <- sprintf("Figures/Dlx/Dlx_Enriched_Mutant_C%s.csv", cluster)
...
genes <- c("Pnoc","Lhx1","Tcf7l2","Gad2","Foxb1","Fgf13","Islr2","Gata2","Emx2","Lhx5","Gadd45g","Ascl1",
           "Nr4a2","Irx5","Irx3","Barhl1","Lmx1a","Dlx5","Sp9","Neurog2","Neurog1","Slc32a1")
