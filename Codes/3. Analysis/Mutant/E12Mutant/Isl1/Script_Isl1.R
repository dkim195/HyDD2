...
# Print the plot
print(p)
postscript(file = "Figures/Isl1/SCENIC_Isl1_eRegulon_Pathway_Regulon_TF.eps", width = 30, height = 15,
...
#Select TE genes
selected_genes <- c("Tcf7l2", "Tbr1", "Lhx2", "Rspo3", "Shox2", "Lhx9", "Neurog2", "Ebf1", "Calb2", "Lhx9",
                    "Nrn1", "Neurod6", "Gbx2", "Cntnap2", "Samd3", "Ap3s1", "Nhlh2", "Epha3", "Uncx",
                    "Cited2", "Rnd2", "Lhx5", "Epha5", "Fezf1", "Neurod1", "Gm28050", "Wnt7b", "Cdkn1a",
                    "Basp1", "Nnat", "Sorbs2", "Mir124-2hg", "Lhx1", "Emx2", "Gm27199", "Insm1", "Tubb3", "Gng8",
                    "Syt13","Nrn1", "Psmd14", "Islr2", "Pou2f2", "Tmem163", "Zic1",
                    "Nhlh2", "Neurod1", "Pou3f2", "Actb", "Rnd2", "Chl1", "Mab21l1", "Fabp5", "Slc17a6",
                    "Nrp2",  "Neurod2", "Eomes")
...
graph_eRegulon <- graph_from_data_frame(filtered_data %>% select(eRegulon_name, Gene, type, importance_TF2G, normalized_rank), directed = TRUE)

# Create the layout to get node positions
ggraph_layout <- create_layout(graph_eRegulon, layout = 'fr')

# Plot the pathway graph based on eRegulon_name
p <- ggraph(ggraph_layout) + 
  geom_edge_link(aes(color = type), 
...
# Print the plot
print(p)
postscript(file = "Figures/Isl1/SCENIC_Isl1_eRegulon_Pathway_Regulon_Thalamic_Em.eps", width = 30, height = 15,
           horizontal = FALSE, onefile = FALSE, paper = "special")
print(p)
dev.off()
...
####Dot plot####
write.csv(filtered_data, "Figures/Isl1/Isl_filtered_data.csv", row.names = FALSE)
...
dat <- melt(graph)
write.csv(dat, file = "Figures/Isl1/dat.csv", row.names = F)
...
ggsave("Figures/Isl1/SCENIC_Isl1_dotplot.eps", plot = dot_plot, width = 20, height = 8, units = "in")
print(dot_plot)
dev.off()
...
library(grDevices)
eps_file <- "Figures/Isl1/Isl1_heatmap.eps"
setEPS()
...
for (cluster in clusters) {
  ...
  file_name <- sprintf("Figures/Isl1/Isl_Enriched_Mutant_C%s.csv", cluster)
...
#png(filename = "Figures/Isl1/Lhx9.png", width = 3000, height = 3000, res = 300)
#print(p)
#dev.off()
...
genes <- c("Emx2","Slc17a6","Tbr1","Eomes","Lhx9", "Tbr1","Wnt8b","Dbx1","Shh","Id3","Pou3f1","Tcf7l2","Hmx2","Prox1")
...
plot_and_save_genes(Mutant, genes, "Figures/Isl1")
