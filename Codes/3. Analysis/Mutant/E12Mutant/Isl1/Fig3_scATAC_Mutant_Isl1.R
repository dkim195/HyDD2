...
####Isl1_scATAC####
#plot
load(file = "scATAC/Files/Isl1_Mutant_adjusted_Final.Robj")
Mutant_adjusted$Cluster_Pass2 <- Mutant_adjusted$predicted.id
DimPlot(Mutant_adjusted, group.by = "Cluster_Pass2")
Idents(Mutant_adjusted) <- "Cluster_Pass2"
Mutant_adjusted <- RenameIdents(Mutant_adjusted, "AntID_ID_TT" = "AntID_ID",
                                "Prethalamus" = "PreThal",
                                "Tuberal (ARC_VMH)" = "Tub")
DimPlot(Mutant_adjusted)
identities <- Idents(Mutant_adjusted)
new_order <- sort(levels(identities))
Idents(Mutant_adjusted) <- factor(identities, levels = new_order)
#11 groups
...
#UMAP
p <- DimPlot(Mutant_adjusted, label = T, cols = group, raster = F)
png(filename = "Figures/Fig3/Isl1/DEG_Isl1_scATAC_raw.png", width = 900, height = 900)
print(p)
dev.off()

#UMAP
p <- DimPlot(Mutant_adjusted, label = , cols = group, raster = F, split.by = "Genotype")+ NoLegend() + NoAxes()
png(filename = "Figures/Fig3/Isl1/DEG_Isl1_scATAC.png", width = 2400, height = 1200, res = 300)
print(p)
dev.off()

#split by Mutant_adjusted
check <- table(Mutant_adjusted@active.ident)
check2 <- table(Mutant_adjusted$Genotype)
Mutant_adjusted <- AddMetaData(Mutant_adjusted, Mutant_adjusted@active.ident, "Cluster")
Mutant_adjusted$CompositeLabel <- paste(Mutant_adjusted$Cluster, Mutant_adjusted$Genotype, sep = "_")
Idents(Mutant_adjusted) <- "CompositeLabel"
...
Idents(Mutant_adjusted) <- factor(Idents(Mutant_adjusted), levels = new_order)

#heatmap
library(pheatmap)
DefaultAssay(Mutant_adjusted) <- 'RNA'
#from DEG
genes <- c("Lhx6", "Prdm12", "Gabra2", "Pcp4", "B230323A14Rik", "Vsnl1", 
           "Foxb1", "Nrsn1", "2810417H13Rik", "Sgol1", "Notch2", "Fam64a", 
           "Slc1a3", "Pmp22", "Gsx1", "Gm11549", "Helt", "Gsx2", "Gadd45g", 
           "Tal2", "Neurog1", "Sstr2", "Eomes", "Rspo3", "Neurog2", "Ttr", 
           "Pf4", "Msx1", "Igfbp7", "Tagln2", "Hmx2", "Calb1", "Hmx3", "RP24-236C17.2", 
           "Syt6", "Bsx", "Gm44644", "Cartpt", "Rgs4", "Otp", "Alcam", "Rxrg", 
           "Sp8", "Meis2", "Sst", "Dlx6os1", "Isl1", "Crnde", "Foxa1", "C130021I20Rik", 
           "Nr4a2", "Lmx1a", "Irx5", "Lmx1b", "Bfsp2", "Ube2ql1", "Irx3", 
           "Nr5a1", "Nr0b1", "Nup62cl", "Chchd10", "Sox14", "Pmch", "Vgll2")
valid_genes <- genes[genes %in% rownames(Mutant_adjusted[["RNA"]])]
Scaling  <- ScaleData(Mutant_adjusted, features = valid_genes)
Avg.Scaling <- AverageExpression(Scaling, assay = "RNA", slot = "counts",
                                 verbose = T) 
transposed_matrix <- t(Avg.Scaling$RNA[valid_genes,])
...
png(filename = "Figures/Fig3/Isl1/scATAC_heatmap_Isl1_DEG_Italic.png", width = 3500, height = 1800, res = 300)
print(heatmp)
dev.off()
#newnames <- lapply(
#  rownames(Avg.Scaling$RNA[valid_genes,]),
#  function(x) bquote(italic(.(x))))

#p <- pheatmap(Avg.Scaling$RNA[valid_genes,], fontsize = 8, scale = "row",
#              fontsize_col = 8, cluster_rows = T,
#              cluster_cols = F, cellwidth = 10, cellheight = 10, angle_col = 45,
#              color = colorRampPalette(c("blue", "white", "red"))(50),
#              labels_row = as.expression(newnames))
#png(filename = "Figures/Fig3/Isl1/scATAC_heatmap_Isl1_DEG_Italic.png", width = 3000, height = 2000)
#print(p)
#dev.off()
...
#              cluster_cols = F, cellwidth = 10, cellheight = 10, angle_col = 45,
#              color = colorRampPalette(c("blue", "white", "red"))(50))
#png(filename = "Figures/Fig3/Isl1/scATAC_heatmap_Isl1_DEG.png", width = 3000, height = 2000)
#print(p)
#dev.off()
...
Avg.Scaling <- AverageExpression(Scaling, assay = "RNA", slot = "counts",
                                 verbose = T) 
# Convert to matrix and handle NA/NaN/Inf
data_matrix <- as.matrix(Avg.Scaling$RNA[valid_genes, ])
data_matrix[is.na(data_matrix) | is.infinite(data_matrix)] <- 0
# Check for zero variance across rows and remove them
zero_variance <- apply(data_matrix, 1, var) == 0
data_matrix <- data_matrix[!zero_variance, ]
if (nrow(data_matrix) > 1) {
  # Generate heatmap ensuring no NA/NaN/Inf values
  clean_data <- t(data_matrix)
  clean_data <- t(scale(t(clean_data)))
  newnames <- lapply(rownames(clean_data), function(x) bquote(italic(.(x))))
  p4 <- pheatmap(clean_data, fontsize = 8,scale = "column",
                 fontsize_col = 8, cluster_rows = FALSE,
                 cluster_cols = FALSE, cellwidth = 10, cellheight = 10, angle_col = 45,
                 color = colorRampPalette(c("blue", "white", "red"))(50),
                 labels_row = as.expression(newnames))
  png(filename = "Figures/Fig3/Isl1/scATAC_heatmap_Isl1_PatternGene_Italic.png",
      width = 8000, height = 1800, res = 300)
  print(p4)
  dev.off()
} else {
  message("Not enough data to create a heatmap after removing zero variance rows.")
}


#if (nrow(data_matrix) > 1) {
#  # Generate heatmap ensuring no NA/NaN/Inf values
#  clean_data <- data_matrix
#  newnames <- lapply(rownames(clean_data), function(x) bquote(italic(.(x))))
#  p4 <- pheatmap(clean_data, fontsize = 8, scale = "row",
...
#                 cluster_cols = FALSE, cellwidth = 10, cellheight = 10, angle_col = 45,
#                 color = colorRampPalette(c("blue", "white", "red"))(50))
#  png(filename = "Figures/Fig3/Isl1/scATAC_heatmap_Isl1_PatternGene.png",
#      width = 3000, height = 2000)
#  print(p4)
#  dev.off()
#} else {
...
Mutant_adjusted <- RenameIdents(Mutant_adjusted, "AntID_ID_TT" = "AntID_ID",
                                "Prethalamus" = "PreThal",
                                "Tuberal (ARC_VMH)" = "Tub")
pl1 <- DimPlot(Mutant_adjusted, combine = F, cols = group, split.by = "Genotype")
# Custom color scale
...
#  coord_fixed()
#png(filename = "Figures/Fig3/Isl1/scATAC_densityheatmap_Isl1.png", width = 1200, height = 1000)
#print(p)
#dev.off()
...
graph <- 100*prop.table(table(Idents(Mutant_adjusted), Mutant_adjusted$Genotype), margin = 1) #margin 1= row, 2 = column
dat <- melt(graph)
...
ggsave(filename = "Figures/Fig3/Isl1/scATAC_densitypercentage_Isl1.eps",
       plot = gplot, width = 20, height = 10, units = "in", device = cairo_ps)

#png(filename = "Figures/Fig3/Isl1/scATAC_densitypercentage_Isl1.png", width = 1200, height = 1000)
#print(p)
#dev.off()
