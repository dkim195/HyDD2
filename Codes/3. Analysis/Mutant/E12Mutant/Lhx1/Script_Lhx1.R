...
selected_eRegulons <- c("Lhx1_extended_+/+", "Lhx1_extended_-/+")

filtered_data <- filtered_data %>%
  filter(eRegulon_name %in% selected_eRegulons)

# Combine 'Lhx1_extended_+/+' and 'Lhx1_extended_-/+' into 'Lhx1'
filtered_data <- filtered_data %>%
...
# Print the plot
print(p)
postscript(file = "Figures/Lhx1/SCENIC_Lhx1_eRegulon_Pathway_Regulon_TF.eps", width = 30, height = 15,
...
####Dot plot####
write.csv(filtered_data, "Figures/Lhx1/Isl_filtered_data.csv", row.names = FALSE)
...
dat <- melt(graph)
write.csv(dat, file = "Figures/Lhx1/dat.csv", row.names = F)
...
ggsave("Figures/Lhx1/SCENIC_Lhx1_dotplot.eps", plot = dot_plot, width = 20, height = 8, units = "in")
print(dot_plot)
dev.off()
...
heatmp <- pheatmap(zscore_data, fontsize = 8, scale = "none",
                   ...
                   labels_row = as.expression(newnames))
library(grDevices)
eps_file <- "Figures/Lhx1/Lhx1_heatmap.eps"
setEPS()
...
clusters <- c("22","25","16","20","12","21","19","23")
for (cluster in clusters) {
  ...
  file_name <- sprintf("Figures/Lhx1/Lhx1_Enriched_Mutant_C%s.csv", cluster)
...
#png(filename = "Figures/Lhx1/Lhx9.png", width = 3000, height = 3000, res = 300)
#print(p)
#dev.off()
...
genes <-  c("Cartpt","Foxa2","Calb2","Rbfox1","Nkx6-2","Gsx2","Top2a","Ascl1","Gbx1",
            "Lhx6","Gad1","Shox2","Tcf7l2","Lhx9","Lhx2","Emx2","Uncx","Tbr1","Lef1","Tcf12","Lmx1a","Fezf1","Nr5a1")
