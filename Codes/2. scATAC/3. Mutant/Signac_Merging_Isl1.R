...
#mutant
load(file = "scATAC/Files/TK44.Robj")
Idents(TK44) <- "orig.ident"
TK44 <- RenameIdents(TK44, "ATAC" = "E12")
TK44 <- AddMetaData(TK44, TK44@active.ident, "Age")
Idents(TK44) <- "orig.ident"
TK44 <- RenameIdents(TK44, "ATAC" = "TK44")
TK44 <- AddMetaData(TK44, TK44@active.ident, "Sample")
Idents(TK44) <- "orig.ident"
TK44 <- RenameIdents(TK44, "ATAC" = "Mutant")
TK44 <- AddMetaData(TK44, TK44@active.ident, "Genotype")
Idents(TK44) <- "Age"

load(file = "scATAC/Files/TK48.Robj")
Idents(TK48) <- "orig.ident"
TK48 <- RenameIdents(TK48, "ATAC" = "E12")
TK48 <- AddMetaData(TK48, TK48@active.ident, "Age")
Idents(TK48) <- "orig.ident"
TK48 <- RenameIdents(TK48, "ATAC" = "TK48")
TK48 <- AddMetaData(TK48, TK48@active.ident, "Sample")
Idents(TK48) <- "orig.ident"
TK48 <- RenameIdents(TK48, "ATAC" = "Mutant")
TK48 <- AddMetaData(TK48, TK48@active.ident, "Genotype")
Idents(TK48) <- "Age"

####Fragment####
DefaultAssay(TK43) <- 'peaks'
DefaultAssay(TK47) <- 'peaks'
DefaultAssay(TK44) <- 'peaks'
DefaultAssay(TK48) <- 'peaks'
...
frags <- Fragments(TK44)  # get list of fragment objects
Fragments(TK44) <- NULL  # remove fragment information from assay
newpath <- "scATAC/Fragments/TK44/fragments.tsv.gz"
frags[[1]] <- UpdatePath(frags[[1]], new.path = newpath)  # update path. Do this for any/all fragment objects in the list
Fragments(TK44) <- frags  # assign update list of fragment objects back to the assay

frags <- Fragments(TK48)  # get list of fragment objects
Fragments(TK48) <- NULL  # remove fragment information from assay
newpath <- "scATAC/Fragments/TK48/fragments.tsv.gz"
frags[[1]] <- UpdatePath(frags[[1]], new.path = newpath)  # update path. Do this for any/all fragment objects in the list
Fragments(TK48) <- frags  # assign update list of fragment objects back to the assay
...
peaks3 <- GetAssayData(TK44, assay = "peaks", slot = "ranges")
peaks4 <- GetAssayData(TK48, assay = "peaks", slot = "ranges")
...
TK47.counts <- FeatureMatrix(
  ...
  features = combined.peaks,
  cells = colnames(TK47))
TK44.counts <- FeatureMatrix(
  fragments = Fragments(TK44),
  features = combined.peaks,
  cells = colnames(TK44))
TK48.counts <- FeatureMatrix(
  fragments = Fragments(TK48),
  features = combined.peaks,
  cells = colnames(TK48))

TK43[['peakunion']] <- CreateAssayObject(counts = TK43.counts)
TK47[['peakunion']] <- CreateAssayObject(counts = TK47.counts)
TK44[['peakunion']] <- CreateAssayObject(counts = TK44.counts)
TK48[['peakunion']] <- CreateAssayObject(counts = TK48.counts)

DefaultAssay(TK43) <- 'peakunion'
DefaultAssay(TK47) <- 'peakunion'
DefaultAssay(TK44) <- 'peakunion'
DefaultAssay(TK48) <- 'peakunion'
...
####Merging####
# Merge the datasets
Mutant <- merge(TK43, y = c(TK47, TK44, TK48),
                add.cell.ids = c("TK43", "TK47", "TK44", "TK48"))
...
save(Mutant, file = "/media/thomaskim/Data/scATAC/Files/Isl1_Mutant_adjusted.Robj")
...
# Identify columns
Mutant_cols <- grep("^(TK44_|TK48_)", colnames(Mutant), value = TRUE)
...
save(Mutant_adjusted, file = "/media/thomaskim/Data/scATAC/Files/Isl1_Mutant_adjusted_Final.Robj")
